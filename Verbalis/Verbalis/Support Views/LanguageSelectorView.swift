    ...
    var body: some View {
            ...
            Group {
                RoundedRectangle(cornerRadius: 20)
                    ...
                    // Outer shadows
                    .shadow(color: self.isButtonPressed ? (colorScheme == .dark ? Color.clear : Color("MetallicBlue")) : Color("DarkShadow"), radius: self.isButtonPressed ? 1 : 4, x: self.isButtonPressed ? -1 : -4, y: self.isButtonPressed ? -1 : -4)
                    .shadow(color: self.isButtonPressed ? (colorScheme == .dark ? Color.clear : Color("MetallicBlue")) : Color("LightShadow"), radius: self.isButtonPressed ? 1 : 4, x: self.isButtonPressed ? 1 : 4, y: self.isButtonPressed ? 1 : 4)
                    // Inner shadows
                    .overlay(
                        RoundedRectangle(cornerRadius: 20)
                            .stroke(colorScheme == .dark ? Color("BGElement") : Color("MetallicBlue"), lineWidth: self.isButtonPressed ? (colorScheme == .dark ? 5 : 7.5) : 0)
                            .blur(radius: colorScheme == .dark ? 3 : 5)
                            .offset(x: 1, y: 1)
                            .mask(RoundedRectangle(cornerRadius: 20).fill(self.isButtonPressed ? (colorScheme == .dark ? LinearGradient(Color("BGElement"), Color.clear) : LinearGradient(Color("MetallicBlue"), Color("BGElement"))) : LinearGradient(Color.clear, Color.clear)))
                    )
                    .overlay(
                        RoundedRectangle(cornerRadius: 20)
                            .stroke(colorScheme == .dark ? Color("BGElement") : Color("MetallicBlue"), lineWidth: self.isButtonPressed ? (colorScheme == .dark ? 5 : 7.5) : 0)
                            .blur(radius: colorScheme == .dark ? 3 : 5)
                            .offset(x: -1, y: -1)
                            .mask(RoundedRectangle(cornerRadius: 20).fill(self.isButtonPressed ? (colorScheme == .dark ? LinearGradient(Color.clear, Color("BGElement")) : LinearGradient(Color("BGElement"), Color("MetallicBlue"))) : LinearGradient(Color.clear, Color.clear)))
                    )
